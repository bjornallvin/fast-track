      ...
      {isAdding && (
        <form onSubmit={handleSubmit} className="mb-6 p-4 bg-gray-50 rounded-lg">
          <textarea
            ...
            className="w-full p-3 border border-gray-300 rounded-md mb-3 min-h-[100px]"
            autoFocus
          />
          <input
            type="text"
          ...
          [...entries].reverse().map((entry) => (
            <div key={entry.id} className="border-l-4 border-indigo-200 pl-4 py-2">
              <div className="flex justify-between items-start mb-2">
                <span className="text-sm text-gray-500">{formatSwedishDateTime(entry.timestamp)}</span>
